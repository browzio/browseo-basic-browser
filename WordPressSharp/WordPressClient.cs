...
namespace WordPressSharp
{
    public class WordPressClient : IDisposable
    {
        ...
        public WordPressClient(WordPressSiteConfig siteConfig, string ip, string port, string username, string password)
        {
            ...
            if(!string.IsNullOrEmpty(ip) && !string.IsNullOrWhiteSpace(ip) && 
                ...
                WordPressService.Proxy = new WebProxy(ip, Convert.ToInt32(port));
            else if (!string.IsNullOrEmpty(ip) && !string.IsNullOrWhiteSpace(ip) &&
                (string.IsNullOrWhiteSpace(port) || string.IsNullOrEmpty(port)))
                WordPressService.Proxy = new WebProxy(ip);

            if(!string.IsNullOrWhiteSpace(username) && !string.IsNullOrEmpty(username) && 
                !string.IsNullOrEmpty(password) && !string.IsNullOrWhiteSpace(password))
            WordPressService.Proxy.Credentials = new NetworkCredential(username, password); 
        }
